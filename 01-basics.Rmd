...
[insert video]
Make sure you follow along with the video
...
We use the # symbol to denote a section heading (with ## being a subsection, and ### being subsubsection, and so on.). Bold text is made by wrapping a sentence in two asterisks `**`, or two underscores `__`, and italic text is made by wrapping it in one `*` or `_`. 

[insert video, writing some text. using a header. writing bold]
...
The little green arrow ![](images/runchunkicon.png) at the top right of the code-chunk allows you to run that whole code-chunk. 

[insert video]

#### Running all code-chunks above the current code-chunk

The little downwards arrow ![](images/runaboveicon.png) enables you to run all the code-chunks above that code-chunk. This is useful to do a quick refresh of your analysis process up to that point. 
